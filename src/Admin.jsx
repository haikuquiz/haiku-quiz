...
import { Settings, Plus, Trash2, LogOut, Bold, Italic, List, Eye, Check, Loader2, ArrowLeft, Lock, Trophy, Flag, Users, Megaphone, Home, LayoutGrid, FileText, Edit3, RefreshCw, Gift, Save, Clock, Calendar, Star, AlertTriangle, ChevronRight, ChevronDown } from 'lucide-react';
...
const sendUserWebhook = async (event, userData) => {
  ...
  try {
    await fetch(webhookUrl, {
      ...
      mode: 'no-cors',
      body: JSON.stringify({ event, oderId: userData.oderId || userData.id, fullName: userData.fullName, username: userData.username, email: userData.email, timestamp: new Date().toISOString() })
    });
...
const recalculateRiddlePoints = async (riddleId, riddle, competition, onLog) => {
  ...
  try {
    ...
    for (const ans of allAnswers) {
      ...
      if (!seenUsers.has(oderId)) { seenUsers.add(oderId); answers.push(ans); }
      else { await updateDoc(ans.ref, { points: 0, isCorrect: false, duplicate: true }); duplicateCount++; }
    }
    if (duplicateCount > 0) log(`   ⚠️ ${duplicateCount} duplicate ignorate`);
    const punti = getEffectivePoints(riddle, competition);
    const getPoints = (pos) => pos === 0 ? (punti.primo || 2) : (punti.altri || 1);
    ...
    log(`   Risposte: ${answers.length}, Corrette: ${correctCount}, Bonus: +${bonus}`);
...
const PointsEditor = ({ punti, onChange, label }) => (
  <div className="p-3 bg-purple-50 rounded-xl border border-purple-200 mb-3">
    <p className="text-sm font-medium text-purple-700 mb-2 flex items-center gap-1"><Trophy size={16} /> {label || 'Punti standard'}</p>
    <div className="grid grid-cols-2 gap-3">
      <div><label className="text-xs text-purple-600">1° classificato</label><input type="number" min="0" value={punti.primo} onChange={e => onChange({ ...punti, primo: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 border rounded-lg text-center" /></div>
      <div><label className="text-xs text-purple-600">Altri corretti</label><input type="number" min="0" value={punti.altri} onChange={e => onChange({ ...punti, altri: parseInt(e.target.value) || 0 })} className="w-full px-3 py-2 border rounded-lg text-center" /></div>
    </div>
  </div>
);

const BonusEditor = ({ bonus, onChange, label }) => (
  <div className="p-3 bg-green-50 rounded-xl border border-green-200 mb-3">
    <p className="text-sm font-medium text-green-700 mb-2 flex items-center gap-1"><Gift size={16} /> {label || 'Punti bonus'}</p>
    <div className="grid grid-cols-3 gap-2">
      ...
      <div><label className="text-xs text-green-600">2-5</label><input type="number" min="0" value={bonus.finoCinque} onChange={e => onChange({ ...bonus, finoCinque: parseInt(e.target.value) || 0 })} className="w-full px-2 py-2 border rounded text-center text-sm" /></div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full">
        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Edit3 size={20} /> Modifica utente</h3>
        <div className="mb-3"><label className="text-sm text-gray-600 mb-1 block">Nome completo</label><input type="text" value={newFullName} onChange={e => setNewFullName(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /></div>
        <div className="mb-3"><label className="text-sm text-gray-600 mb-1 block">Nickname</label><input type="text" value={newUsername} onChange={e => setNewUsername(e.target.value)} className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl" /></div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => onSave(user.id, newUsername.trim(), newFullName.trim())} disabled={saving || newUsername.trim().length < 3} className="flex-1 bg-purple-600 text-white py-3 rounded-xl font-semibold disabled:bg-gray-400 flex items-center justify-center gap-2">
...
const CompetitionRiddlesSection = ({ competition, riddles, onViewAnswers, onEditRiddle, onDeleteRiddle }) => {
  const [expanded, setExpanded] = useState(false);
  const compRiddles = riddles.filter(r => r.competitionId === competition.id).sort((a, b) => {
  ...
  const now = new Date();
  const activeCount = compRiddles.filter(r => { const s = r.dataInizio?.toDate?.() || new Date(r.dataInizio); const e = r.dataFine?.toDate?.() || new Date(r.dataFine); return now >= s && now <= e; }).length;
    ...
    <div className="bg-white rounded-2xl mb-3 overflow-hidden">
      <button onClick={() => setExpanded(!expanded)} className="w-full p-4 flex items-center justify-between hover:bg-gray-50">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${activeCount > 0 ? 'bg-green-100' : 'bg-gray-100'}`}>
            <Flag className={activeCount > 0 ? 'text-green-600' : 'text-gray-500'} size={20} />
          </div>
          <div className="text-left">
            <h4 className="font-semibold text-gray-800">{competition.nome}</h4>
            <p className="text-xs text-gray-500">{compRiddles.length} quiz {activeCount > 0 && <span className="text-green-600">• {activeCount} attivi</span>}</p>
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t">
          {compRiddles.length === 0 ? <p className="text-gray-500 text-center py-4">Nessun quiz</p> : (
            <div className="space-y-2 mt-3">
              {compRiddles.map(r => {
                ...
                const hasCustom = r.puntiCustom || r.bonusCustom;
                return (
                  <div key={r.id} className={`p-3 rounded-xl border ${isActive ? 'bg-green-50 border-green-200' : isPast ? 'bg-gray-50' : 'bg-blue-50 border-blue-200 border-dashed'}`}>
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-medium text-gray-800">{r.titolo}</span>
                          {hasCustom && <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">Custom</span>}
                          {isActive && <span className="text-xs bg-green-500 text-white px-2 py-0.5 rounded-full">Live</span>}
                        </div>
                        <p className="text-xs text-gray-500 mt-1">{formatDateTime(start)} → {formatDateTime(end)}</p>
                        <p className="text-xs text-purple-600 mt-1">Risposta: {r.risposta}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => onViewAnswers(r)} className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Eye size={16} /></button>
                        <button onClick={() => onEditRiddle(r)} className="p-2 bg-purple-50 text-purple-600 rounded-lg"><Edit3 size={16} /></button>
                        <button onClick={() => onDeleteRiddle(r)} className="p-2 bg-red-50 text-red-600 rounded-lg"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  </div>
                );
...
const RiddleAnswersView = ({ riddle, competition, answers, users, onBack, onRecalculate, recalculating }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6">
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded-xl border">
        <p className="text-xs text-gray-500 uppercase font-semibold mb-2">Domanda:</p>
      ...
      <div className="mb-4 p-4 bg-purple-50 rounded-xl">
        ...
        <p className="text-xs text-gray-500 mt-1">Punti: 1° {punti.primo}pt | Altri {punti.altri}pt</p>
        {(bonus.uno > 0 || bonus.finoCinque > 0 || bonus.seiDieci > 0) && <p className="text-xs text-green-600 mt-1">Bonus: 1 +{bonus.uno} | 2-5 +{bonus.finoCinque} | 6-10 +{bonus.seiDieci}</p>}
        {riddle.correctCount !== undefined && <p className="text-xs text-blue-600 mt-1">Corrette: {riddle.correctCount}</p>}
        {(riddle.puntiCustom || riddle.bonusCustom) && <p className="text-xs text-yellow-600 mt-1">⭐ Punteggio custom</p>}
      </div>
...
const Admin = () => {
  ...
  const handleRecalculatePoints = async (riddle) => {
    ...
    const result = await recalculateRiddlePoints(riddle.id, riddle, comp, logFn);
    if (result.success) {
  ...
  const handleUpdateCompetition = async () => {
    ...
    try {
      const regolamento = regolamentoEditorRef.current?.innerHTML || '';
      await updateDoc(doc(db, 'competitions', editingCompetition.id), { nome: editingCompetition.nome, descrizione: editingCompetition.descrizione || '', regolamento, puntiDefault: editingCompetition.puntiDefault, bonusDefault: editingCompetition.bonusDefault });
  ...
  const handleUpdateUser = async (userId, newUsername, newFullName) => {
    if (!userId || newUsername.length < 3) { showMsg('Nickname min 3 caratteri'); return; }
    setSubmitting(true);
    try {
      const userDoc = await getDoc(doc(db, 'users', userId));
      const oldData = userDoc.data();
      await updateDoc(doc(db, 'users', userId), { username: newUsername, fullName: newFullName });
      const scoresSnap = await getDocs(query(collection(db, 'competitionScores'), where('oderId', '==', userId)));
      for (const scoreDoc of scoresSnap.docs) await updateDoc(scoreDoc.ref, { username: newUsername });
      await sendUserWebhook('user_updated_by_admin', { oderId: userId, fullName: newFullName, username: newUsername, email: oldData?.email });
      setEditingUser(null);
  ...
  const handleDeleteUserComplete = async (userToDelete) => {
    ...
    try {
      ...
      for (const d of scoresSnap.docs) {
        ...
        if (scoreData.competitionId) { const compRef = doc(db, 'competitions', scoreData.competitionId); const compDoc = await getDoc(compRef); if (compDoc.exists() && (compDoc.data().participantsCount || 0) > 0) await updateDoc(compRef, { participantsCount: compDoc.data().participantsCount - 1 }); }
  ...
  const activeComps = competitions.filter(c => isCompetitionActive(c));
  const pastComps = competitions.filter(c => isCompetitionPast(c));
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4">
        {activeTab === 'dashboard' && (<div className="space-y-4"><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><button onClick={() => setActiveTab('competitions')} className="bg-white rounded-2xl p-4 text-center hover:shadow-md transition-shadow"><Flag className="mx-auto text-purple-500 mb-2" size={28} /><p className="text-2xl font-bold">{competitions.length}</p><p className="text-sm text-gray-500">Gare</p></button><button onClick={() => setActiveTab('riddles')} className="bg-white rounded-2xl p-4 text-center hover:shadow-md transition-shadow"><LayoutGrid className="mx-auto text-blue-500 mb-2" size={28} /><p className="text-2xl font-bold">{riddles.length}</p><p className="text-sm text-gray-500">Quiz</p></button><button onClick={() => setActiveTab('users')} className="bg-white rounded-2xl p-4 text-center hover:shadow-md transition-shadow"><Users className="mx-auto text-green-500 mb-2" size={28} /><p className="text-2xl font-bold">{users.length}</p><p className="text-sm text-gray-500">Utenti</p></button><button onClick={() => setActiveTab('announcements')} className="bg-white rounded-2xl p-4 text-center hover:shadow-md transition-shadow"><Megaphone className="mx-auto text-orange-500 mb-2" size={28} /><p className="text-2xl font-bold">{announcements.length}</p><p className="text-sm text-gray-500">Avvisi</p></button></div>{activeComps.length > 0 && (<div className="bg-white rounded-2xl p-5"><h3 className="font-bold mb-3 flex items-center gap-2"><Star className="text-yellow-500" /> Gare in corso</h3>{activeComps.map(c => (<div key={c.id} onClick={() => startEditCompetition(c)} className="p-4 bg-green-50 rounded-xl border border-green-200 cursor-pointer mb-2 hover:bg-green-100"><div className="flex justify-between items-start"><div><h4 className="font-semibold text-green-800">{c.nome}</h4><p className="text-sm text-green-600">{c.participantsCount || 0} partecipanti • {riddles.filter(r => r.competitionId === c.id).length} quiz</p></div><ChevronRight className="text-green-400" size={20} /></div></div>))}</div>)}</div>)}
        {activeTab === 'competitions' && (<div className="space-y-4"><button onClick={() => setCreatingCompetition(true)} className="w-full bg-purple-600 text-white py-4 rounded-2xl font-semibold flex items-center justify-center gap-2"><Plus size={20} /> Nuova Gara</button>{activeComps.length > 0 && (<div><h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><Star className="text-green-500" /> In corso ({activeComps.length})</h3>{activeComps.map(c => (<div key={c.id} className="bg-white rounded-2xl p-4 mb-2 border-2 border-green-200"><div className="flex justify-between items-start"><div className="flex-1 cursor-pointer" onClick={() => startEditCompetition(c)}><h4 className="font-semibold text-gray-800">{c.nome}</h4><p className="text-xs text-gray-500">{formatDate(c.dataInizio)} - {formatDate(c.dataFine)} • {c.participantsCount || 0} iscritti</p></div><div className="flex gap-2"><button onClick={() => startEditCompetition(c)} className="text-purple-600 p-2"><Edit3 size={18} /></button><button onClick={() => setConfirmDelete({ type: 'competition', id: c.id, name: c.nome })} className="text-red-500 p-2"><Trash2 size={18} /></button></div></div></div>))}</div>)}{pastComps.length > 0 && (<div><h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><Clock className="text-gray-500" /> Passate ({pastComps.length})</h3>{pastComps.map(c => (<div key={c.id} className="bg-white rounded-2xl p-4 mb-2 opacity-75"><div className="flex justify-between items-start"><div className="flex-1 cursor-pointer" onClick={() => startEditCompetition(c)}><h4 className="font-semibold text-gray-600">{c.nome}</h4><p className="text-xs text-gray-400">{formatDate(c.dataInizio)} - {formatDate(c.dataFine)}</p></div><div className="flex gap-2"><button onClick={() => startEditCompetition(c)} className="text-purple-600 p-2"><Edit3 size={18} /></button><button onClick={() => setConfirmDelete({ type: 'competition', id: c.id, name: c.nome })} className="text-red-500 p-2"><Trash2 size={18} /></button></div></div></div>))}</div>)}</div>)}
        {activeTab === 'riddles' && (<div className="space-y-4"><button onClick={() => setCreatingRiddle(true)} className="w-full bg-purple-600 text-white py-4 rounded-2xl font-semibold flex items-center justify-center gap-2"><Plus size={20} /> Nuovo Quiz</button><h3 className="font-bold text-gray-800">Quiz per gara</h3>{competitions.map(comp => (<CompetitionRiddlesSection key={comp.id} competition={comp} riddles={riddles} onViewAnswers={viewAnswers} onEditRiddle={startEditRiddle} onDeleteRiddle={(r) => setConfirmDelete({ type: 'riddle', id: r.id, name: r.titolo })} />))}</div>)}
        ...
        {activeTab === 'users' && (<div className="bg-white rounded-2xl p-5"><h3 className="font-bold mb-3">Utenti ({users.length})</h3><div className="space-y-2 max-h-[60vh] overflow-y-auto">{users.map(u => (<div key={u.id} className="p-4 bg-gray-50 rounded-xl border flex justify-between items-center"><div className="flex-1"><p className="font-medium">{u.username}</p>{u.fullName && <p className="text-sm text-gray-600">{u.fullName}</p>}<p className="text-sm text-gray-500">{u.email}</p><p className="text-xs text-gray-400">Registrato: {formatDate(u.createdAt)}</p></div><div className="flex gap-2"><button onClick={() => setEditingUser(u)} className="text-purple-600 p-2"><Edit3 size={18} /></button><button onClick={() => setDeletingUser(u)} className="text-red-500 p-2"><Trash2 size={18} /></button></div></div>))}</div></div>)}
      </div>
